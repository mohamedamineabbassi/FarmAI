...
cv2.destroyAllWindows()
    ...
    # =========================
    now = time.time()
    if now - getattr(save_unknown, 'last_upload_time', 0) > 1.0:
        try:
            cv2.imwrite("temp_stream.jpg", frame)
            with open("temp_stream.jpg", "rb") as f:
                requests.post(UPLOAD_URL, files={'file': f}, data={'cameraId': str(CAMERA_ID)}, timeout=2)
                save_unknown.last_upload_time = now
        except Exception:
            pass
